 ...
 */
function ControlledDropdownFieldConfigPanel({
  initialField,
  onUpdate = jest.fn(),
}: {
  initialField: DropdownFieldConfig;
  onUpdate?: jest.Mock;
}) {
  const [field, setField] = useState(initialField);
  function handleUpdate(updated: DropdownFieldConfig) {
    setField(updated);
    onUpdate(updated);
  }
  return <DropdownFieldConfigPanel field={field} onUpdate={handleUpdate} />;
}

describe('DropdownFieldConfigPanel', () => {
  it('renders the base config panel inputs', () => {
    render(<DropdownFieldConfigPanel field={makeField()} onUpdate={jest.fn()} />);
  ...
  it('updating a base field calls onUpdate with updated base field and unchanged options', async () => {
    ...
    render(<ControlledDropdownFieldConfigPanel initialField={makeField()} onUpdate={onUpdate} />);
  ...
  it('adding an option calls onUpdate with the new option appended', async () => {
    ...
    const [updated] = onUpdate.mock.calls[0] as [DropdownFieldConfig];
