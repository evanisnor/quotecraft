...
import type { CheckboxFieldConfig } from '@/shared/config';
import { CheckboxFieldConfigPanel } from './CheckboxFieldConfigPanel';

function makeField(overrides: Partial<CheckboxFieldConfig> = {}): CheckboxFieldConfig {
  return {
    id: 'field-1',
    type: 'checkbox',
    label: 'Package Size',
 ...
 */
function ControlledCheckboxFieldConfigPanel({
  initialField,
  onUpdate = jest.fn(),
}: {
  initialField: CheckboxFieldConfig;
  onUpdate?: jest.Mock;
}) {
  const [field, setField] = useState(initialField);
  function handleUpdate(updated: CheckboxFieldConfig) {
    setField(updated);
    onUpdate(updated);
  }
  return <CheckboxFieldConfigPanel field={field} onUpdate={handleUpdate} />;
}

describe('CheckboxFieldConfigPanel', () => {
  it('renders the base config panel inputs', () => {
    render(<CheckboxFieldConfigPanel field={makeField()} onUpdate={jest.fn()} />);
  ...
  it('updating a base field calls onUpdate with updated base field and unchanged options', async () => {
    ...
    render(<ControlledCheckboxFieldConfigPanel initialField={makeField()} onUpdate={onUpdate} />);
  ...
  it('adding an option calls onUpdate with the new option appended', async () => {
    ...
    const [updated] = onUpdate.mock.calls[0] as [CheckboxFieldConfig];
