...
// MountAuth registers authentication routes on the server's private route group.
func (s *Server) MountAuth(reg Registrar) {
	s.privateGroup.Post("/auth/register", registerHandler(reg))
}
