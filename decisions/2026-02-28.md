...
## Task: BLDR-US1-A004 — Implement API client module for authenticated requests

**Requirements:** 1.1.3, 1.2.2, 1.2.4

### Decisions

**`ApiClient` interface in `shared/api` with `createApiClient` factory**

The `ApiClient` interface provides `get<T>`, `post<T>`, `put<T>`, and `delete` methods. The factory `createApiClient(baseUrl, token, fetcher?)` bundles the three injection points previously threaded through every entity API function, feature hook, and page component. This eliminates repeated `(baseUrl, token, fetcher)` parameters across the codebase.

**`delete` handled separately from `get/post/put` in the factory**

The `get/post/put` internal `request<T>` helper always parses JSON and unwraps the envelope. `delete` is different: 204 has no response body, and only error responses have a JSON envelope. The separate implementation handles this: on success it returns void without parsing; on error it tries to parse JSON for the message, with fallback to `'Unknown error'`.

**`StubApiClient` placed in `shared/api/testing.ts`**

`StubApiClient` implements `ApiClient` from `shared`, so it belongs in `shared/api/testing.ts` alongside `StubFetch`. This avoids the FSD violation of feature/page test files importing from an entity's internal segment path. Test files import from `@/shared/api/testing` (full path, not the public barrel).

**`createApiClient` memoized in `app/(dashboard)/dashboard/page.tsx`**

`DashboardPage` lists `client` as a `useEffect` dependency. Without `useMemo`, a new client object is created on every render, producing a new object reference each time and triggering repeated `listCalculators` calls. The fix: `useMemo(() => createApiClient(API_BASE_URL, token), [token])`.

**Entity API functions simplified to single `client: ApiClient` parameter**

`listCalculators(client)`, `createCalculator(client)`, `deleteCalculator(client, id)` — the entity functions are now thin wrappers that delegate to the client. The `ApiEnvelope<T>` type is no longer duplicated per-entity; it is imported from `@/shared/api`.

---

## Task: BLDR-US2-A002 — Implement shared field configuration panel

**Requirements:** 1.3.8

### Decisions

**`BaseFieldConfig` in `shared/config/calculatorConfig.ts`**

The base interface for all field types lives in `shared/config` since it is shared between the editor (which writes config) and the widget (which reads it). It includes `id`, `type`, `label`, `helpText?` (optional per REQ 1.3.8), `required`, and `variableName`. Type-specific config properties will extend this interface in A003–A007.

**`generateVariableName` in `shared/lib/variableName.ts`**

A pure utility function that converts a human-readable label into a snake_case slug. Rules: lowercase, replace non-alphanumeric runs with `_`, collapse consecutive underscores, trim leading/trailing underscores. Empty or all-symbol inputs return `""`. Lives in `shared/lib` with its own `index.ts` barrel export.

**Auto-sync variable name behavior in `FieldConfigPanel`**

When the label changes, the variable name is auto-updated only if it is still "in sync" with the old label (`field.variableName === generateVariableName(field.label)`). Once the user manually edits the variable name (breaking sync), label changes no longer override it. This is implemented in the `handleLabelChange` handler entirely within the controlled component.

**`FieldConfigPanel` is fully controlled — no internal state**

All state management is deferred to the parent. The component receives `field: BaseFieldConfig` and `onUpdate: (updated: BaseFieldConfig) => void`. The parent is responsible for propagating updates.

**`helpText` optional with `?? ''` fallback on the input**

`helpText?: string` in the type; `value={field.helpText ?? ''}` in the JSX. This keeps the input always controlled (never `undefined`) without requiring callers to supply an empty string.

**Tests use `ControlledFieldConfigPanel` wrapper for userEvent interactions**

Since `FieldConfigPanel` is a controlled component, `userEvent.type()` on a bare render without state propagation would type into a component that never updates. A `ControlledFieldConfigPanel` wrapper with `useState` is defined in the test file itself — scoped to where it's needed, not promoted to a shared `testing.ts`.

### Technical Challenges

**`fireEvent` vs `userEvent` for controlled text inputs**

The initial implementation used `fireEvent.change()` which directly dispatches a synthetic event with any `target.value`. Code review required switching to `userEvent` for consistency with project conventions. For a controlled component, this requires a stateful test wrapper so each character typed propagates back to the component. The `ControlledFieldConfigPanel` wrapper pattern solves this cleanly.
