...
## Task: INFR-US4-A003 — Implement logout with server-side token invalidation

**Requirements:** 1.1.5

### Decisions

**SHA-256 hash computed in `Service.Logout`, not in the repository**

`Logout` hashes the raw token with `sha256.Sum256` and encodes it as lowercase hex before passing the hash to `SessionDeleter.DeleteSession`. The repository layer only receives and operates on the pre-computed hash string — it never sees the raw token. This keeps the hashing logic in the service (domain) layer, consistent with `Register`'s approach of keeping token generation and hashing co-located with the business logic. The repository remains a thin data-access adapter.

**`DeleteSession` is idempotent — zero rows affected is not an error**

The `DELETE FROM sessions WHERE token_hash = $1` query may affect zero rows if the session has already expired or was never valid. `DeleteSession` discards `RowsAffected` from `sql.Result` and does not treat zero rows as an error. This follows the HTTP semantics of `DELETE`: a resource that does not exist is indistinguishable from one that was already deleted. The caller (logout handler) gets a clean nil error and responds 204 No Content.

**`Logouter` interface defined in `server` package (consumer)**

`Logout(ctx context.Context, token string) error` is defined as a `Logouter` interface in `internal/server/auth.go`, following the interfaces-at-consumer convention. `AuthService` composite interface now embeds all three: `Registrar`, `Authenticator`, `Logouter`. `MountAuth(svc AuthService)` registers all three routes.

**`extractBearerToken` returns empty string on missing/malformed header**

A thin helper `extractBearerToken(r *http.Request) string` checks for the `Bearer ` prefix and trims it. If the Authorization header is absent or does not begin with `Bearer `, it returns `""`. The handler treats an empty result as a 401 Unauthorized. This keeps the token extraction logic testable in isolation and avoids repetition across future protected endpoints.

**`logoutHandler` responds 204 No Content on success**

Logout invalidates a session but returns no body — 204 No Content is the correct HTTP status for a successful action with no response payload.

**`SessionDeleter` interface separate from `SessionWriter`**

`DeleteSession` is defined on a new `SessionDeleter` interface rather than expanding `SessionWriter`. `Register` and `Login` only need to create sessions; `Logout` only needs to delete them. ISP: each consumer takes only the capability it needs. `NewService` now accepts four parameters: `userWriter`, `userReader`, `sessions SessionWriter`, `sessionDeleter SessionDeleter`. `PostgresSessionRepository` satisfies both; `main.go` passes `sessionRepo` for both session params.

### Technical Challenges

**`NewService` arity mismatch after agent ran out of context**

The Golang agent ran out of tokens mid-implementation and left two `NewService(...)` calls in `auth_test.go` with three arguments instead of four (missing `&stubSessionDeleter{}`), and the same issue in `main.go` (missing the fourth `sessionRepo` argument). This caused a compilation failure. The fixes were applied manually using the Edit tool on the specific lines identified from the compiler output.

---

## Task: INFR-US4-A004 — Implement auth middleware that validates session tokens on protected routes

**Requirements:** 1.1.3, 1.1.5, 1.1.6

### Decisions

**`SessionReader` interface separate from `SessionWriter` and `SessionDeleter`**

`GetSession` is defined on a new `SessionReader` interface rather than expanding existing session interfaces. `ValidateToken` needs to read sessions; `Register`/`Login` only write; `Logout` only deletes. ISP: each consumer takes exactly the capability it needs. `PostgresSessionRepository` satisfies all three. `NewService` now accepts five parameters — `sessionReader SessionReader` added as the fifth parameter, after `sessionDeleter`. `main.go` passes `sessionRepo` for all three session params.

**`ErrSessionNotFound` and `ErrInvalidSession` as distinct sentinels**

`ErrSessionNotFound` is returned by `SessionReader.GetSession` when no row matches — it is a repository-level signal. `ErrInvalidSession` is returned by `Service.ValidateToken` for all token validation failures (not found, expired) — it is a service-level signal. The distinction allows the service to map `ErrSessionNotFound` → `ErrInvalidSession` while still propagating unexpected repository errors (wrapping in `"looking up session: %w"`) separately. The handler can then check for `auth.ErrInvalidSession` to produce a 401 without also catching unexpected errors.

**`TokenValidator` interface defined in the `server` package (consumer)**

Per the interfaces-at-consumer convention, `TokenValidator` is defined in `internal/server/auth.go`. `AuthService` now embeds `Registrar`, `Authenticator`, `Logouter`, and `TokenValidator`. The `auth.Service` struct satisfies `TokenValidator` without the auth package knowing anything about the server package.

**`authUserKey{}` unexported context key type**

An unexported struct type is used as the context key so only code within the `server` package can construct the same key. This prevents collisions with other packages and is idiomatic Go. Since `authUserKey` is unexported, `TestUserIDFromContext_Set` uses `context.WithValue(ctx, authUserKey{}, ...)` directly — this is accessible in tests because they are in `package server` (not `package server_test`).

**`UserIDFromContext` returns `("", false)` for empty string**

The function checks both the type assertion `ok` and that the resulting string is non-empty. An empty string stored under the key (e.g., if buggy code set it) is treated the same as an absent key. This prevents handlers from proceeding with an empty user ID as if it were authenticated.

**`RequireAuth` middleware calls `LoggerFrom` for unexpected errors**

Unexpected validator errors (not `ErrInvalidSession`) are logged with `LoggerFrom(r.Context()).Error(...)` before returning a 500. This keeps the pattern consistent with the handler-level error logging convention already established for `registerHandler`, `loginHandler`, and `logoutHandler`.

**`Authenticated(validator)` method returns `chi.Router` for protected route mounting**

Rather than a middleware registration function, the server exposes `Authenticated(TokenValidator) chi.Router`. Callers receive a router they can call `.Get()`, `.Post()`, etc. on. Routes registered on this router automatically have `RequireAuth` in their middleware chain because it is applied via `g.Use(RequireAuth(validator))` inside a `privateGroup.Group(...)` callback. The lambda captures the inner `g chi.Router` and assigns it to `r` which is returned. This pattern is the standard chi approach for sub-routers with middleware.

### Technical Challenges

**`newServiceForTest` parameter count growing with each task**

Adding `sessionReader SessionReader` as the fifth parameter required updating all eight existing `newServiceForTest` call sites in `auth_test.go`. The calls to `NewService` in non-`newServiceForTest` tests (for `Register`, `EmailConflict`, etc.) also needed updating. All callers were updated in one pass using targeted Edit tool calls. The function remains package-private and test-only.

**`stubSessionReader{}` default returning `(nil, nil)` would panic**

A default `stubSessionReader{}` (zero value) returns `(nil, nil)` from `GetSession`. If `ValidateToken` were called with this stub, the `sess.ExpiresAt.Before(...)` line would panic on a nil pointer dereference. This is intentional: tests that do not exercise `ValidateToken` (Register, Login, Logout tests) use `&stubSessionReader{}` because they will never call `GetSession`. Tests that exercise `ValidateToken` always configure the stub explicitly with either a session or an error. The risk of accidental nil pointer panic is acceptable because any such test failure would be an obvious panic rather than a silent wrong-answer failure.

---

## Task: INFR-US5-A001 — Implement create calculator endpoint (`POST /v1/calculators`)

**Requirements:** 1.2.1, 1.9.3, 1.9.4

### Decisions

**`internal/calculator` package with hexagonal boundaries**

The calculator domain lives in `internal/calculator`. `Calculator` is the domain model. `Creator` is the port (interface) defined at the adapter boundary — it is consumed by `Service` but implemented by `PostgresCalculatorRepository`. The `Creator` interface is defined in the same file as the `Service` (the consumer), not in `postgres.go` (the implementor). This follows the interfaces-where-consumed convention from the Golang skill.

**`CalculatorCreator` and `CalculatorService` defined in the `server` package**

Per the interfaces-at-consumer convention, `CalculatorCreator` and `CalculatorService` are defined in `internal/server/calculator.go`. The `calculator.Service` struct satisfies these interfaces without importing the server package. `CalculatorService` embeds `CalculatorCreator` as a composite interface that will grow as subsequent INFR-US5 tasks add more operations (list, get, update, delete, duplicate, config).

**`stubCalculatorService` added to `stubs_test.go`**

Following the pattern established for `stubAuthService` and `stubPinger`, the test stub for `CalculatorService` lives in `internal/server/stubs_test.go`. This keeps all server-package stubs in one discoverable location rather than scattered across per-test files.

**`createCalculatorResponse` omits `Config` and `IsDeleted`**

The create response returns only `id`, `created_at`, and `updated_at`. The config is always `{}` at creation time (no value for the client), and `is_deleted` is always `false`. This matches the acceptance criteria: "creates a calculator, returns its UUID". Additional fields can be added to the response in future tasks when the GET endpoint is introduced and a richer response shape is needed.

**`MountCalculators` takes both `TokenValidator` and `CalculatorService`**

`MountCalculators` calls `s.Authenticated(validator)` internally to get a protected sub-router, then mounts the calculator handler on it. The validator and service are both injected at mount time. In `main.go`, `authService` is passed as the validator — `auth.Service` satisfies `TokenValidator` — and `calcService` is passed as the service. This keeps `main.go` wiring minimal and explicit.

**`PostgresCalculatorRepository.CreateCalculator` uses `RETURNING` clause**

Rather than separate `INSERT` + `SELECT` queries, the INSERT uses a `RETURNING` clause to fetch all columns in a single round-trip. This is idiomatic PostgreSQL and avoids a race condition where the row could be modified between insertion and retrieval. The `config` column is scanned into `[]byte` — JSONB from PostgreSQL is returned as raw JSON bytes, which is the correct type for `Calculator.Config`.

### Technical Challenges

**No significant challenges.** The implementation followed established patterns from the auth package. The main care was ensuring `stubCalculatorService` was placed in `stubs_test.go` rather than `calculator_test.go` to align with the reusable-stubs-in-testing-files convention.
